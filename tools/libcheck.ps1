...
function Build() {
	msbuild.exe "$RootDir\src\DotNetXri\DotNetXri.csproj" /p:Configuration=$Configuration
}

function Generate-Metadata($Version) {
	Push-Location $LibCheckTmpDir
	& ".\libcheck.exe" -store "DotNetXri.dll" $Version -full "$BinDir\$Configuration"
	Pop-Location
}
...
function ShadowCopy-Libcheck() {
	...
	# directory so that as we git checkout other versions of DotNetXri,
